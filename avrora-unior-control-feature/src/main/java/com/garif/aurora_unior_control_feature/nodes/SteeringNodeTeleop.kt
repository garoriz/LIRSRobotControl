...
class SteeringNodeTeleop(private val topic: String, private val context: Context): AbstractNodeMain() {
    ...
    override fun onStart(connectedNode: ConnectedNode?) {
        ...
        publisherTimer.schedule(object : TimerTask() {
            override fun run() {
                movable?.setSteering(float64)
                publisher?.publish(float64)
                Log.d(
                    "Publisher  in SteeringNodeTeleop",
                    float64.toString()
                )
            }
        }, 0L, 80L)
